...
def get_multi_by_user(db: Session, *, user_id: str, skip: int = 0, limit: int = 10):
    """Lấy danh sách các profile của một user."""
    query = text("SELECT * FROM investment_profiles WHERE user_id = :user_id ORDER BY created_at DESC LIMIT :limit OFFSET :skip")
    result = db.execute(query, {"user_id": user_id, "limit": limit, "offset": skip})
