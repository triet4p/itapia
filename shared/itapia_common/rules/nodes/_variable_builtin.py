...
register_node_by_spec(nms.VAR_D_SUPPORT_1_PRICE, NodeSpec(
    ...
    params={'path': 'technical_report.daily_report.sr_report.supports.0.level'}
))
register_node_by_spec(nms.VAR_D_RESISTANCE_1_PRICE, NodeSpec(
    node_class=NumericalVarNode,
    description='Price level of the nearest resistance',
    return_type=SemanticType.PRICE,
    params={'path': 'technical_report.daily_report.sr_report.resistances.0.level'}
))
...
register_node_by_spec(nms.VAR_D_PATTERN_1_SCORE, NodeSpec(
    ...
    params={'path': 'technical_report.daily_report.pattern_report.top_patterns.0.score'}
))
...
register_node_by_spec(nms.VAR_FC_TB_PREDICTION, NodeSpec(
    CategoricalVarNode, 'Forecasted trade outcome (-1: loss, 0: timeout, 1: win)', SemanticType.NUMERICAL,
    # Giá trị đã là -1, 0, 1 nên không cần mapping
    {'path': 'forecasting_report.forecasts.0.prediction.0', 'mapping': {-1: -1.0, 0: 0.0, 1: 1.0}}
))

# --- C.2. Từ task 5-Day Distribution Regression (forecasts[1]) ---
register_node_by_spec(nms.VAR_FC_5D_MEAN_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted mean price change in 5 days (%)', SemanticType.PERCENTAGE,
    # Giả định thay đổi 5% trong 5 ngày là lớn
    {'path': 'forecasting_report.forecasts.1.prediction.0', 'source_range': (-5, 5), 'target_range': (-1, 1)}
))
register_node_by_spec(nms.VAR_FC_5D_STD_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted price volatility (std) in 5 days (%)', SemanticType.VOLATILITY,
    # Giả định độ lệch chuẩn 5% là rất cao
    {'path': 'forecasting_report.forecasts.1.prediction.1', 'source_range': (0, 5), 'target_range': (0, 1)}
))
register_node_by_spec(nms.VAR_FC_5D_MAX_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted max potential upside in 5 days (%)', SemanticType.PERCENTAGE,
    {'path': 'forecasting_report.forecasts.1.prediction.3', 'source_range': (0, 10), 'target_range': (0, 1)}
))
register_node_by_spec(nms.VAR_FC_5D_MIN_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted max potential downside in 5 days (%)', SemanticType.PERCENTAGE,
    {'path': 'forecasting_report.forecasts.1.prediction.2', 'source_range': (-10, 0), 'target_range': (-1, 0)}
))
register_node_by_spec(nms.VAR_FC_5D_Q25_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted q25 price change in 5 days (%)', SemanticType.PERCENTAGE,
    # Giả định thay đổi 5% trong 5 ngày là lớn
    {'path': 'forecasting_report.forecasts.1.prediction.4', 'source_range': (-5, 5), 'target_range': (-1, 1)}
))
register_node_by_spec(nms.VAR_FC_5D_Q75_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted q75 price change in 5 days (%)', SemanticType.PERCENTAGE,
    # Giả định thay đổi 5% trong 5 ngày là lớn
    {'path': 'forecasting_report.forecasts.1.prediction.5', 'source_range': (-5, 5), 'target_range': (-1, 1)}
))

# --- C.3. Từ task 20-Day Distribution Regression (forecasts[2]) ---
register_node_by_spec(nms.VAR_FC_20D_MEAN_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted mean price change in 20 days (%)', SemanticType.PERCENTAGE,
    # Giả định thay đổi 10% trong 20 ngày là lớn
    {'path': 'forecasting_report.forecasts.2.prediction.0', 'source_range': (-10, 10), 'target_range': (-1, 1)}
))
register_node_by_spec(nms.VAR_FC_20D_STD_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted price volatility (std) in 20 days (%)', SemanticType.VOLATILITY,
    # Giả định độ lệch chuẩn 10% là rất cao
    {'path': 'forecasting_report.forecasts.2.prediction.1', 'source_range': (0, 10), 'target_range': (0, 1)}
))
register_node_by_spec(nms.VAR_FC_20D_MAX_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted max potential upside in 20 days (%)', SemanticType.PERCENTAGE,
    {'path': 'forecasting_report.forecasts.2.prediction.3', 'source_range': (0, 15), 'target_range': (0, 1)}
))
register_node_by_spec(nms.VAR_FC_20D_MIN_PCT, NodeSpec(
    NumericalVarNode, 'Forecasted max potential downside in 20 days (%)', SemanticType.PERCENTAGE,
    {'path': 'forecasting_report.forecasts.2.prediction.2', 'source_range': (-15, 0), 'target_range': (-1, 0)}
))
