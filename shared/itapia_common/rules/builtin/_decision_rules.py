...
def _create_rule_1_trend_following() -> Rule:
    ...
    logic_tree = create_node(nms.OPR_DIV2, children=[sum_of_trends, create_node(nms.CONST_NUM(2.0))])
...
def _create_rule_4_news_sentiment_balance() -> Rule:
    ...
    denominator_sum_div = create_node(nms.OPR_ADD2, children=[denominator_sum, nms.CONST_NUM(1.0)])
    
    logic_tree = create_node(nms.OPR_DIV2, children=[numerator, denominator_sum])
...
def _create_rule_10_high_impact_news_direction() -> Rule:
    ...
    logic_tree = create_node(nms.OPR_IF_THEN_ELSE, children=[
        cond_has_high_impact, latest_news_sentiment, create_node(nms.CONST_NUM(0.0))
    ])
