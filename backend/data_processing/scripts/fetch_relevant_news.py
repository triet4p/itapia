...
def _extract_news_data(
    tickers: list[str], sleep_time: int = 5, max_news: int = 10
) -> list[dict]:
...
def _transform_element(element: dict):
    ...
    if not content:  # Check for both None and empty dictionary
        raise FetchException(
            f"News with id {transformed['news_uuid']} do not have 'content'"
        )
    ...
    if not transformed["title"]:  # Title is required
        raise FetchException(
            f"News with id {transformed['news_uuid']} do not have 'title'"
        )

    transformed["summary"] = content.get(
        "summary", ""
    ...
    if pub_date_str:
        ...
        raise FetchException(
            f"News with id {transformed['news_uuid']} do not have 'pubDate'"
        )
    ...
    transformed["provider"] = (
        provider_info.get("displayName", "") if provider_info else ""
    )

    # Safely get 'link'
    click_through_info = content.get("clickThroughUrl")  # Could be dict or None
    transformed["link"] = (
        click_through_info.get("url", "") if click_through_info else ""
    )
...
if __name__ == "__main__":
    ...
    full_pipeline(metadata_service, news_service, max_news=15, sleep_time=5)
