...
from .utils import DEFAULT_RETURN_DATE, FetchException

logger = ITAPIALogger(id="Prices Batch Processor")


def _extract_raw_data(
    tickers: List[str], start_collect_date: datetime, end_collect_date: datetime
) -> pd.DataFrame:
...
def _reconstruct_table(
    raw_df: pd.DataFrame, numeric_type: Literal["float32", "float64"] = "float32"
):
...
def _handle_missing_data(
    df: pd.DataFrame, start_date: datetime, end_date: datetime, features: List[str]
):
    ...
    filtered_df = df[
        (df["collect_date"] >= pd_start_date) & (df["collect_date"] <= pd_end_date)
    ]
...
def full_pipeline(
    ...
    try:
        ...
        end_date = datetime(
            now_date.year, now_date.month, now_date.day, 22, 0, 0, tzinfo=timezone.utc
        ) - timedelta(days=delta_day)

        logger.info(
            f"Start collect from {start_date} to {end_date} for {len(tickers)} tickers from Yahoo Finance API..."
        )
        ...
        prices_service.add_daily_prices(
            data=selected_df, unique_cols=["collect_date", "ticker"]
        )
