...
def test_convert_parts_to_json_string():
    ...
    # Create mock data
    mock_data = MockProfileData(name="test", value={"nested": "data"})
...
def test_get_profile_by_id_success():
    ...
    with pytest.MonkeyPatch().context() as m:
        m.setattr(
            "app.services.profiles.profile_crud.get_by_id",
            lambda db, profile_id: mock_row,
        )
...
def test_get_profile_by_id_not_found():
    ...
    with pytest.MonkeyPatch().context() as m:
        m.setattr(
            "app.services.profiles.profile_crud.get_by_id", lambda db, profile_id: None
        )
        ...
        with pytest.raises(NoDataError) as exc_info:
            service.get_profile_by_id(
                profile_id="nonexistent_profile", user_id="user123"
            )
...
def test_get_profiles_by_user():
    ...
    with pytest.MonkeyPatch().context() as m:
        m.setattr(
            "app.services.profiles.profile_crud.get_multi_by_user",
            lambda db, user_id: mock_rows,
        )
...
def test_create_profile_failure():
    ...
    with pytest.MonkeyPatch().context() as m:
        m.setattr(
            "app.services.profiles.profile_crud.create", lambda db, profile_data: None
        )
...
def test_update_profile_success():
    ...
    with pytest.MonkeyPatch().context() as m:
        ...
        update_data = ProfileUpdate(
            profile_name="Updated Test Profile", description="An updated test profile"
        )

        # Test profile update
        result = service.update_profile(
            profile_id="profile123", user_id="user123", profile_in=update_data
        )
...
def test_update_profile_no_changes():
    ...
    with pytest.MonkeyPatch().context() as m:
        ...
        m.setattr("app.services.profiles.profile_crud.update", mock_update)
        m.setattr(
            service, "_convert_parts_to_json_string", mock_convert_parts_to_json_string
        )
        ...
        assert result.profile_id == "profile123"
        assert (
            update_called is False
        )  # Update shouldn't be called when there are no changes
...
def test_remove_profile_failure():
    ...
    with pytest.MonkeyPatch().context() as m:
        m.setattr(
            service,
            "get_profile_by_id",
            lambda profile_id, user_id: mock_profile_entity,
        )
        m.setattr(
            "app.services.profiles.profile_crud.remove", lambda db, profile_id: None
        )
