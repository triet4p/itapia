...
class ProfileService:
    ...
    def get_profile_by_name(self, *, profile_name: str, user_id: str) -> ProfileEntity:
        ...
        Raises:
            ...
            AuthError: If the user doesn't own the profile
        """
        row = profile_crud.get_by_name(
            self.db, profile_name=profile_name, user_id=user_id
        )
        entity = ProfileEntity.model_validate(row) if row else None

        if entity is None:
            raise NoDataError(
                f"Not found profile with name {profile_name} for user {user_id}"
            )
        if user_id != entity.user_id:
            raise AuthError(
                f"Profile with id {entity.profile_id} can not accessed by user {user_id}"
            )
    ...
    def update_profile(
        self, *, profile_id: str, user_id: str, profile_in: ProfileUpdate
    ) -> ProfileEntity:
        ...
        """
        profile_to_update = self.get_profile_by_id(
        ...
        updated_row = profile_crud.update(
            self.db, profile_id=profile_id, update_data=update_data
        )
    ...
    def remove_profile(self, *, profile_id: str, user_id: str) -> ProfileEntity:
        ...
        """
        profile_to_delete = self.get_profile_by_id(
            profile_id=profile_id, user_id=user_id
        )
