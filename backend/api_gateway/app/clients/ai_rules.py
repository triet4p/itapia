...
from itapia_common.schemas.api.rules import (
...
async def get_ready_rules(
    ...
    try:
        ...
        response = await ai_rules_client.get(
            f"/rules", params={"purpose": purpose.value}
        )
        response.raise_for_status()
...
async def get_nodes(
    node_type: NodeType, purpose: SemanticType = SemanticType.ANY
) -> list[NodeResponse]:
    try:
        print(f"Get for url {ai_rules_client.base_url}/rules/nodes")
        response = await ai_rules_client.get(
            f"/rules/nodes",
            params={"purpose": purpose.value, "node_type": node_type.value},
        )
        response.raise_for_status()
