...
async def get_suggest_config(profile: ProfileRequest) -> QuantitivePreferencesConfigResponse:
    try:
        print(f'Get for url {ai_personal_client.base_url}/personal/suggested_config')
        body = profile.model_dump()
        response = await ai_personal_client.post(f"/personal/suggested_config", json=body)
