...
class MOEADEvoEngine(BaseEvoEngine):
    ...
    def _init_decomposer(self):
        self.decomposer = DecompositionManager(
            self.num_objs, self.num_divisions, self.neighborhood_size
        )
    ...
    def add_crossover_opr(
        self, crossover_opr: CrossoverOperator[MOEADIndividual], init_score: float = 1.0
    ) -> Self:
    ...
    def add_mutation_opr(
        self, mutation_opr: MutationOperator[MOEADIndividual], init_score: float = 1.0
    ) -> Self:
    ...
    def _update_reference_point(self, individual: MOEADIndividual):
        ...
        self.reference_points = np.maximum(
            self.reference_points, np.array(individual.fitness)
        )

    def _select_parents_from_neighborhood(
        self, subproblem_index: int
    ) -> Tuple[MOEADIndividual, MOEADIndividual]:
    ...
    def _apply_mutation(
        self,
        mutation_opr: MutationOperator[MOEADIndividual],
        ind: MOEADIndividual,
        pm: float,
        force: bool = False,
    ) -> MOEADIndividual | None:
        """
        ...
        for i in range(self.pop_size):
            ...
            if self._random.random() < self.pc:
                ...
                if pairs:
                    ...
                    improvement += max(
                        0.0,
                        c.flatten_fitness(self.adaptive_obj_extractor)
