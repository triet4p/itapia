...
from ..dominance import crowding_distance_assignment, non_dominated_sorting
from ..objective import SingleObjectiveExtractor
...
from ..pop import DominanceIndividual, Population
...
class NSGA2EvoEngine(BaseEvoEngine):
    ...
    def set_selection_opr(
        self, selection_opr: SelectionOperator[DominanceIndividual]
    ) -> Self:
        self.selection_opr = selection_opr
        return self

    def set_replacement_opr(
        self, replacement_opr: ReplacementOperator[DominanceIndividual]
    ) -> Self:
        self.replacement_opr = replacement_opr
        return self

    def add_crossover_opr(
        self,
    ...
    def set_adaptive_obj_extractor(
        self, adaptive_obj_extractor: SingleObjectiveExtractor
    ) -> Self:
    ...
    def _apply_mutation(
        self,
        mutation_opr: MutationOperator[DominanceIndividual],
        ind: DominanceIndividual,
        pm: float,
        force: bool = False,
    ) -> DominanceIndividual | None:
        """
    ...
    def _gen_offs_each_epoch(
        self, last_pop: List[DominanceIndividual], pc: float, pm: float
    ) -> Population:
        ...
        while len(offs) < len(last_pop):
            ...
            if self._random.random() < pc:
                ...
                if pairs:
                    ...
                    worst_parent_fitness = min(
                        p1.flatten_fitness(self.adaptive_obj_extractor),
                        p2.flatten_fitness(self.adaptive_obj_extractor),
                    )
                    improvement = 0.0
                    improvement += max(
                        0.0,
                        c1.flatten_fitness(self.adaptive_obj_extractor)
                        - worst_parent_fitness,
                    )
                    improvement += max(
                        0.0,
                        c2.flatten_fitness(self.adaptive_obj_extractor)
                        - worst_parent_fitness,
                    )
        ...
        offs_pop = Population(
            max_population_size=len(offs), ind_cls=DominanceIndividual
        )
    ...
    def _run_each_gen(self, pop_size: int, pc: float, pm: float) -> None:
        ...
        logger.info(
            f"Calculating fitness for offs population of {self.pop.population_size} individuals..."
        )
        # offs.cal_fitness(self.evaluator, self.obj_extractor) #Remove
        self.archived.population.extend(
            self._random.sample(
                offs.population, k=min(self.archive_each_gen, len(offs.population))
            )
        )

        # Create new population
        logger.info("Performing survival selection...")
        new_pop = self.replacement_opr(
    ...
    def run(self, **kwargs) -> List[DominanceIndividual]:
        ...
        logger.info(
            f"Calculating fitness for initial population of {self.pop.population_size} individuals..."
        )
        self.pop.cal_fitness(self.evaluator, self.obj_extractor)
        self.archived = Population(
            max_population_size=cfg.MAX_ARCHIVED_RULES, ind_cls=self.pop.ind_cls
        )
        ...
        for gen in range(1, num_gen + 1):
            ...
            logger.info(
                f"Generation {gen} classification complete. Pareto front size: {len(self.pareto_front)}"
            )
