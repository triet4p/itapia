...
def replace_node(
    root: _TreeNode, old_node: _TreeNode, new_node: _TreeNode
) -> _TreeNode:
...
def get_concreate_type(
    required_type: SemanticType,
    random_ins: random.Random,
    available_types: Set[SemanticType],
) -> SemanticType:
    ...
    safe_concreate_types = tuple(
        t for t in required_type.concreates if t in available_types
    )
...
def grow_tree(
    ...
    any_return_operators = (
        operators_by_type.get(SemanticType.ANY, []) if current_depth > 1 else []
    )
    any_numeric_return_operators = (
        operators_by_type.get(SemanticType.ANY_NUMERIC, []) if current_depth > 1 else []
    )

    possible_operators = (
        specific_operators + any_return_operators + any_numeric_return_operators
    )
    ...
    if (
        is_max_depth
        or not has_operators
        or (current_depth > 1 and random_ins.random() < cfg.INIT_TERMINAL_PROB)
    ):
        ...
        if not possible_terminals:
            raise ValueError(
                f"Could not find a terminal for required type '{required_type}' at depth {current_depth}."
            )
        ...
        for arg_type in set(args_types):
            if arg_type.concreates:
                if op_spec.return_type == arg_type:
                    ...
                    resolved_types[arg_type] = get_concreate_type(
                        arg_type, random_ins, available_types
                    )
