...
class DecompositionManager(Stateful):
    ...
    def __init__(self, num_objs: int,
                 num_divisions: int,
    ...
    def num_vectors(self) -> int:
        ...
        if self.problem_weight_vectors is None:
            raise ValueError("Weight vectors must be generated before calculating neighborhoods.")
    ...
    def generate_weight_vectors(self) -> 'DecompositionManager':
        ...
        assert num_generated == expected_num, \
    ...
    def calculate_neighborhoods(self) -> 'DecompositionManager':
        ...
        if T >= num_vectors:
            ...
            self.neighborhoods = np.array([
                np.roll(np.arange(num_vectors), -i-1)[:num_vectors-1] for i in range(num_vectors)
            ])
            return self
        ...
        diff = self.problem_weight_vectors[:, np.newaxis, :] - self.problem_weight_vectors[np.newaxis, :, :]
    ...
    def scalar_tchebycheff(self, ind: MOEADIndividual, 
                           neighbor_idx: int,
