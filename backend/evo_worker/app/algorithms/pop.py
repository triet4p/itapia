from typing import Any, Dict, Generic, List, Self, Tuple, Type, TypeVar

from app.algorithms.objective import (
    AcceptedObjective,
    MultiObjectiveExtractor,
    ObjectiveExtractor,
    SingleObjectiveExtractor,
)
...
class Individual(Stateful):
    ...
    def cal_fitness(
        self, evaluator: Evaluator, obj_extractor: ObjectiveExtractor
    ) -> AcceptedObjective:
...
class DominanceIndividual(Individual):
    ...
    def cal_fitness(
        self, evaluator: Evaluator, obj_extractor: MultiObjectiveExtractor
    ) -> Tuple[float, ...]:
...
class MOEADIndividual(Individual):
    ...
    def fallback_state(self) -> Dict[str, Any]:
        state = super().fallback_state
        state.update({"problem_idx": self.problem_idx})
        return state
...
class Population(Stateful, Generic[IndividualType]):
    ...
    def __init__(self, ind_cls: Type[IndividualType], max_population_size: int = None):
        ...
        self.max_population_size: int = (
            max_population_size if max_population_size else 1000000
        )
    ...
    def extend_pop(
        self, population: List[IndividualType], auto_clip: bool = True
    ) -> bool:
