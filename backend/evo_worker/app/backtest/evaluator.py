...
class Evaluator(ABC):
    ...
    def add_evaluate_scenario(
        self, action_mapper: _BaseActionMapper, weight: float = 1.0
    ):
...
class SingleContextEvaluator(Evaluator):
    ...
    def __init__(
        ...
        if context.status != "READY_SERVE":
            raise ValueError(
                f"Context for '{context.ticker}' is not in 'READY' state. Please load data into memory first."
            )
    ...
    def _generate_actions(
        self, rule: Rule, action_mapper: _BaseActionMapper
    ) -> Dict[datetime, Action]:
        ...
        for report in self.historical_reports:
            report_date = datetime.fromtimestamp(
                report.generated_timestamp, tz=timezone.utc
            )
            if not (start_date <= report_date <= end_date):
                continue
            try:
                score_result = rule.execute(report)
                action = action_mapper.map_action(score_final=score_result)
                if action.action_type != "HOLD":
                    actions_dict[report_date] = action
            except Exception as e:
                logger.warn(
                    f"Rule execution failed for '{rule.name}' on report date {report_date.date()}. Error: {e}"
                )
                continue

        return actions_dict

    def _evaluate_each_scenario(
        self, rule: Rule, scenario: EvaluateScenario
    ) -> PerformanceMetrics:
        ...
        """
        logger.debug(
            f"Evaluating rule '{rule.name}' for ticker '{self.context.ticker}' with scenario {str(scenario.action_mapper)}..."
        )
    ...
    def _aggerate_obj_from_scenarios(
        self, scenario_results: Dict[EvaluateScenario, PerformanceMetrics]
    ) -> PerformanceMetrics:
        ...
        for scenario, res in scenario_results.items():
            ...
            for attr in final_metrics_dict.keys():
                final_metrics_dict[attr] += (
                    res_dict[attr] * scenario.weight / total_weight
                )
...
class MultiContextEvaluator(Evaluator):
    ...
    def _evaluate_single_context_sync(
        self, context: BacktestContext, rule: Rule
    ) -> PerformanceMetrics:
        ...
        try:
            ...
            logger.warn(
                f"Evaluation failed for rule '{rule.name}' on ticker '{context.ticker}': {e}"
            )
    ...
    async def _evaluate_with_semaphore(
        ...
        async with self.semaphore:
            ...
            return await asyncio.to_thread(
                self._evaluate_single_context_sync, context, rule
            )
    ...
    async def evaluate_async(self, rule: Rule) -> PerformanceMetrics:
        ...
        logger.debug(
            f"Evaluating rule '{rule.name}' on {len(self.contexts)} tickers with concurrency limit {PARALLEL_MULTICONTEXT_LIMIT}..."
        )

        tasks = [
            self._evaluate_with_semaphore(context, rule) for context in self.contexts
        ]
        ...
        if not results:
            logger.warn(
                f"All evaluations failed for rule '{rule.name}'. Returning worst-case fitness."
            )
            worst_metrics = PerformanceMetrics()
            return worst_metrics
        ...
        if self.aggregation_method == "mean":
            ...
            metrics_series = metrics_df.apply(
                lambda col: trim_mean(col, proportiontocut=0.1)
            )
        ...
        logger.debug(
            f"Rule '{rule.name}' evaluation complete. Aggregated objectives: {aggregated_metrics}"
        )
    ...
    def evaluate(self, rule: Rule) -> PerformanceMetrics:
        ...
        try:
            ...
            raise RuntimeError(
                "evaluate_sync cannot be called from within a running asyncio event loop."
            )
        except RuntimeError:
