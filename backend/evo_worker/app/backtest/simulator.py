...
from .trade import EXIT_REASON_TYPE, OpenPosition, Trade
...
class BacktestSimulator:
    ...
    def run(self) -> List[Trade]:
        ...
        """
        logger.debug(
            f"Running simulation on {len(self.ohlcv_df)} historical data points..."
        )
        ...
        if self.current_position:
            ...
            logger.debug(
                f"Position still open at the end of simulation. Closing at last price {last_close_price:.2f}"
            )
            self._close_position(last_date, last_close_price, "TIME_STOP")

        logger.debug(
            f"Simulation finished. Total trades executed: {len(self.trade_log)}"
        )
    ...
    def _process_new_signal(
        self, current_date: datetime, daily_row: pd.Series, action: Action
    ):
        ...
        """
        entry_price = daily_row["close"] * (
            1 + self.trading_fee_pct
        )  # Entry price including fees/slippage
        ...
        if action.action_type == "BUY" and self.current_position is None:
            ...
            logger.debug(
                f"Opened LONG position on {current_date.date()} at {entry_price:.2f}"
            )
        ...
        # Open SHORT position if no position exists
            ...
            logger.debug(
                f"Opened SHORT position on {current_date.date()} at {entry_price:.2f}"
            )

    def _close_position(
        self, exit_date: datetime, exit_price: float, reason: EXIT_REASON_TYPE
    ):
