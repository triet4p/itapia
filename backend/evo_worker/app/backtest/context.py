...
class BacktestContext:
    ...
    async def prepare_data_async(self) -> None:
        ...
        if self.status != "IDLE":
            logger.warn(
                f"Preparation for ticker {self.ticker} already started. Current status: {self.status}"
            )
            return
        ...
        try:
            ...
            self.ohlcv_df = self.data_preparer.get_daily_ohlcv_for_ticker(
                self.ticker, limit=5000
            )
            ...
            if not timestamps_to_request:
                logger.warn(
                    f"No backtest points selected for ticker {self.ticker}. Marking as ready."
                )
                self.status = "READY_LOAD"
                self.data_ready_event.set()
                return
    ...
    async def _poll_for_completion(self) -> None:
        ...
        logger.info(
            f"Polling for backtest generation status every {polling_interval} seconds..."
        )

        while True:
            try:
                response = await api_caller.check_backtest_generation_status(
                    self.job_id
                )
                logger.info(
                    f"Current backtest generation of job {self.job_id} status: {response.status}"
                )
    ...
    def _choose_reports_from_selector(
        self, reports: List[QuickCheckAnalysisReport]
    ) -> List[QuickCheckAnalysisReport]:
        """Select reports based on selector criteria."""
        if not reports:
            return []

        report_dates = pd.to_datetime(
            [r.generated_timestamp for r in reports], unit="s", utc=True
        )
        target_dates = pd.to_datetime(
            self.selector.get_points_as_timestamps(), unit="s", utc=True
        )

        report_series = pd.Series(range(len(reports)), index=report_dates)
        report_indices = report_dates.get_indexer(
            target_dates, method="nearest", tolerance=pd.Timedelta(timedelta(days=3))
        )
    ...
    async def load_data_into_memory(self, max_reports: Optional[int] = None) -> None:
        ...
        if self.status not in ["READY_LOAD", "READY_SERVE"]:
            raise BacktestError(
                f"Data is not available to load for ticker {self.ticker}. Status: {self.status}"
            )
        ...
        ohlcv_task = loop.run_in_executor(
            None, self.data_preparer.get_daily_ohlcv_for_ticker, self.ticker, 5000
        )
        reports_task = loop.run_in_executor(
            None, self.data_preparer.get_backtest_reports_for_ticker, self.ticker
        )

        self.ohlcv_df, historical_reports = await asyncio.gather(
            ohlcv_task, reports_task
        )
        ...
        logger.info(
            f"Successfully loaded {len(self.historical_reports)} reports and OHLCV data for {self.ticker}."
        )
...
class BacktestContextManager:
    ...
    async def prepare_all_contexts(self) -> None:
        ...
        logger.info(
            f"Preparing backtest contexts for {len(tickers)} tickers with a concurrency limit of {concurrency_limit}..."
        )
    ...
    def log_summary(self) -> None:
        ...
        ready_count = len(
            [ctx for ctx in self.contexts.values() if ctx.status == "READY_LOAD"]
        )
        failed_count = len(
            [ctx for ctx in self.contexts.values() if ctx.status == "FAILED"]
        )
        preparing_count = len(
            [ctx for ctx in self.contexts.values() if ctx.status == "PREPARING"]
        )
