...
class EvoRuleCRUD:
    ...
    def get_all_rules_by_evo(
        self, rule_status: str, evo_run_id: str
    ) -> Sequence[RowMapping]:
        # Postgres JSONB query: `->>` extracts field as text
        stmt = text(
            """SELECT rule_id, name, description, purpose, rule_status, created_at, updated_at, root, evo_run_id, metrics
                    FROM public.evo_rules WHERE rule_status = :rule_status AND evo_run_id = :evo_run_id;"""
        )

        results = self.db.execute(
            stmt, {"rule_status": rule_status, "evo_run_id": evo_run_id}
        )
...
class EvoRunCRUD:
    ...
    def get_evo_run(self, evo_run_id: str) -> Optional[RowMapping]:
        stmt = text(
            """SELECT run_id, status, config, fallback_state, algorithm
