...
def _create_rule_1_deep_value_screener() -> Rule:
    ...
    """
    cond_rsi_oversold = create_node(
        nms.OPR_LT,
        children=[create_node(nms.VAR_D_RSI_14), create_node(nms.CONST_RSI_OVERSOLD)],
    )
    cond_long_trend_up = create_node(
        nms.OPR_EQ,
        children=[
            create_node(nms.VAR_D_TREND_LONGTERM_DIR),
            create_node(nms.CONST_SEMANTIC(SemanticType.TREND, SemanticLevel.HIGH)),
        ],
    )

    # Use AND, it will return 1.0 if both are true, 0.0 otherwise.
    logic_tree = create_node(
        nms.OPR_AND, children=[cond_rsi_oversold, cond_long_trend_up]
    )

    return _build_opportunity_rule(
        "RULE_O_01_DEEP_VALUE",
        "Deep Value Screener",
        "Scores highly for oversold stocks within a long-term uptrend.",
        logic_tree,
    )
...
def _create_rule_2_trend_reversal_screener() -> Rule:
    ...
    cond_long_trend_down = create_node(
        nms.OPR_EQ,
        children=[
            create_node(nms.VAR_D_TREND_LONGTERM_DIR),
            create_node(nms.CONST_SEMANTIC(SemanticType.TREND, SemanticLevel.LOW)),
        ],
    )

    is_reversal_signal = create_node(
        nms.OPR_AND, children=[cond_mid_trend_up, cond_long_trend_down]
    )
    ...
    return _build_opportunity_rule(
        "RULE_O_02_TREND_REVERSAL",
        "Trend Reversal Screener",
        "Scores positively for early signs of a trend reversal from bearish to bullish.",
        logic_tree,
    )
...
def _create_rule_3_forecast_upside_potential() -> Rule:
    ...
    return _build_opportunity_rule(
        ...
        "Scores based on the normalized forecasted max upside potential.",
        logic_tree,
    )
...
def _create_rule_4_bullish_pattern_screener() -> Rule:
    ...
    """
    cond_is_bull_pattern = create_node(
        nms.OPR_GTE,
        children=[
    ...
    return _build_opportunity_rule(
        "RULE_O_04_BULLISH_PATTERN",
        "Bullish Pattern Screener",
        "Scores based on the existence and strength of a bullish pattern.",
        logic_tree,
    )
...
def _create_rule_5_news_catalyst_screener() -> Rule:
    ...
    logic_tree = create_node(
        nms.OPR_MUL2, children=[positive_news_score, high_impact_news_score]
    )

    return _build_opportunity_rule(
        "RULE_O_05_NEWS_CATALYST",
        "News Catalyst Screener",
        "Scores based on the density of positive and high-impact news.",
        logic_tree,
    )
