...
def _build_rule(
    ...
    root_node = create_node(node_name=nms.OPR_TO_DECISION_SIGNAL, children=[logic_tree])
...
def _create_rule_1_trend_following() -> Rule:
    ...
    sum_of_trends = create_node(
        nms.OPR_ADD2, children=[mid_term_trend, long_term_trend]
    )
    sum_of_trends_num = create_node(nms.OPR_TO_NUMERICAL, children=[sum_of_trends])

    logic_tree = create_node(
        nms.OPR_DIV2, children=[sum_of_trends_num, create_node(nms.CONST_NUM(2.0))]
    )

    return _build_rule(
        "RULE_D_01_TREND_FOLLOW",
        "Trend Following Score",
        "Calculates a score based on the alignment of mid and long-term trends.",
        logic_tree,
    )
...
def _create_rule_2_rsi_momentum() -> Rule:
    ...
    return _build_rule(
        "RULE_D_02_RSI_MOMENTUM",
        "RSI Momentum Score",
        "Uses the normalized RSI value directly as a momentum signal.",
        logic_tree,
    )
...
def _create_rule_3_ml_forecast_confidence() -> Rule:
    ...
    return _build_rule(
        "RULE_D_03_ML_CONFIDENCE",
        "ML Forecast Confidence",
        "Calculates a score based on the confidence (Prob_Win - Prob_Loss) of the ML model.",
        logic_tree,
    )
...
def _create_rule_4_news_sentiment_balance() -> Rule:
    ...
    denominator_sum_div = create_node(
        nms.OPR_ADD2, children=[denominator_sum_num, create_node(nms.CONST_NUM(1.0))]
    )

    logic_tree = create_node(
        nms.OPR_DIV2, children=[numerator_num, denominator_sum_div]
    )

    return _build_rule(
        "RULE_D_04_NEWS_BALANCE",
        "News Sentiment Balance",
        "Calculates a score based on the ratio of positive to negative news.",
        logic_tree,
    )
...
def _create_rule_5_trend_strength_confirmation() -> Rule:
    ...
    return _build_rule(
        "RULE_D_05_TREND_STRENGTH",
        "Trend Strength Confirmation",
        "Amplifies trend signal by its strength.",
        logic_tree,
    )
...
def _create_rule_6_pattern_sentiment() -> Rule:
    ...
    return _build_rule(
        "RULE_D_06_PATTERN_SENTIMENT",
        "Pattern Sentiment Signal",
        "Uses the sentiment of the most prominent pattern directly as a signal.",
        logic_tree,
    )
...
def _create_rule_7_mean_reversion_fading() -> Rule:
    ...
    cond_overbought = create_node(
        nms.OPR_GT,
        children=[create_node(nms.VAR_D_RSI_14), create_node(nms.CONST_RSI_OVERBOUGHT)],
    )
    cond_oversold = create_node(
        nms.OPR_LT,
        children=[create_node(nms.VAR_D_RSI_14), create_node(nms.CONST_RSI_OVERSOLD)],
    )

    inner_if = create_node(
    ...
    logic_tree = create_node(
        nms.OPR_IF_THEN_ELSE,
        children=[cond_overbought, create_node(nms.CONST_NUM(-1.0)), inner_if],
    )

    return _build_rule(
        "RULE_D_07_MEAN_REVERSION_FADE",
        "Mean Reversion Fading",
        "Generates a contrarian signal based on RSI overbought/oversold levels.",
        logic_tree,
    )
...
def _create_rule_8_forecast_potential() -> Rule:
    ...
    return _build_rule(
        "RULE_D_08_FC_POTENTIAL",
        "Forecasted Potential Score",
        "Balances the forecasted max upside against the max downside.",
        logic_tree,
    )
...
def _create_rule_9_intraday_momentum() -> Rule:
    ...
    return _build_rule(
        "RULE_D_09_INTRADAY_MOMENTUM",
        "Intraday Breakout Momentum",
        "Signal based on intraday opening range breakout status.",
        logic_tree,
    )
...
def _create_rule_10_high_impact_news_direction() -> Rule:
    ...
    cond_has_high_impact = create_node(
        nms.OPR_GTE,
        children=[
            create_node(nms.VAR_NEWS_SUM_NUM_HIGH_IMPACT),
            create_node(nms.CONST_SEMANTIC(SemanticType.SENTIMENT, SemanticLevel.HIGH)),
        ],
    )
    latest_news_sentiment = create_node(nms.VAR_NEWS_SUM_NUM_POSITIVE)
    latest_news_sentiment_num = create_node(
        nms.OPR_TO_NUMERICAL, children=[latest_news_sentiment]
    )
    logic_tree = create_node(
        nms.OPR_IF_THEN_ELSE,
        children=[
            cond_has_high_impact,
            latest_news_sentiment_num,
            create_node(nms.CONST_NUM(0.0)),
        ],
    )

    return _build_rule(
        "RULE_D_10_HIGH_IMPACT_NEWS",
        "High-Impact News Direction",
        "Signal is driven by the sentiment of the latest news, only if high-impact news exists.",
        logic_tree,
    )
