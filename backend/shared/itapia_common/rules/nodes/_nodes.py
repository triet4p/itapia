...
class _TreeNode(ABC):
    ...
    def __eq__(self, value: object) -> bool:
        ...
        if not isinstance(value, _TreeNode):
            return False
        return (
            self.node_name == value.node_name and self.return_type == value.return_type
        )
    ...
    def __init__(
        ...
        if use_normalize:
            if source_range is None or target_range is None:
                raise ValueError(
                    "use_normalize=True requires source range and target range"
                )
...
class NumericalVarNode(VarNode):
    ...
    def encode(self, raw_value: float) -> float:
        ...
        """
        return normalize(
            raw_value, self.default_value, self.source_range, self.target_range
        )
...
class FunctionalOperatorNode(OperatorNode):
    ...
    def __init__(
        ...
        """
        super().__init__(
            node_name, description, num_child, return_type, args_type, children
        )
