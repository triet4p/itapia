...
class BaseSHAPExplaination(BaseModel):
    ...
    prediction_outcome: float = Field(
...
class SHAPExplaination(BaseModel):
    ...
    explaination: BaseSHAPExplaination = Field(
...
class _BaseTaskMetadata(BaseModel):
    ...
    problem_id: Literal["triple-barrier", "ndays-distribution"] = Field(
        ..., description="To discriminate each metadata type"
    )
...
class SingleTaskForecastReport(BaseModel):
    ...
    task_metadata: Union[TripleBarrierTaskMetadata, NDaysDistributionTaskMetadata] = (
        Field(..., discriminator="problem_id", description="Metadata of task.")
    )
    prediction: List[float] = Field(
        ..., description="Model forecast results (in percent form multiplied by 100)."
    )
    units: Literal["percent", "category"] = Field(
