...
class _BaseTrendReport(BaseModel):
    ...
    ma_direction: Literal["uptrend", "downtrend", "undefined"] = Field(
        ..., description="Direction using MA indicators"
    )
    ma_status: Literal["positive", "negative", "undefined"] = Field(
...
class MidTermTrendReport(_BaseTrendReport):
    ...
    adx_direction: Literal["uptrend", "downtrend", "undefined"] = Field(
...
class OverallStrengthTrendReport(BaseModel):
    ...
    strength: Literal["strong", "moderate", "weak", "undefined"] = Field(
...
class TrendReport(BaseModel):
    ...
    primary_focus: Literal["mid-term", "long-term"] = Field(
        default="mid_term", description="Primary focus of analysis"
    )
    midterm_report: MidTermTrendReport = Field(..., description="Mid-Term Trend Report")
    longterm_report: LongTermTrendReport = Field(
        ..., description="Long-Term Trend Report"
    )
    overall_strength: OverallStrengthTrendReport = Field(
...
class PatternObj(BaseModel):
    ...
    pattern_type: Literal["chart", "candlestick"] = Field(
...
class PatternReport(BaseModel):
    ...
    prominence_pct: float = Field(
        default=0.015, description="Prominence percents to find peaks"
    )
    distance: int = Field(
        default=5, description="Distance of neighbor points to find peaks"
    )
    lookback_period: int = Field(
        default=5, description="Lookback period for candlestick patterns"
    )
    num_top_patterns: int = Field(
        default=4, description="Number of top patterns to recognize"
    )
...
class DailyAnalysisReport(BaseModel):
    ...
    key_indicators: KeyIndicators = Field(
