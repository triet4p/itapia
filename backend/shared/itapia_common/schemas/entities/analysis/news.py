...
class ImpactAssessmentReport(BaseModel):
    ...
    words: List[str] = Field(
        ..., default_factory=list, description="List of evidence impact words"
    )
...
class SingleNewsAnalysisReport(BaseModel):
    ...
    sentiment_analysis: SentimentAnalysisReport = Field(
        ..., description="Sentiment Analysis Report"
    )
    ner: NERReport = Field(..., description="NER Report")
    impact_assessment: ImpactAssessmentReport = Field(
        ..., description="Impact Assessment Report"
    )
    keyword_highlighting_evidence: KeywordHighlightingReport = Field(
