...
InvestmentHorizon = Literal[
    "short_term", "mid_term", "long_term"  # Under 1 year  # 1-5 years  # Over 5 years
]
...
InvestmentKnowledge = Literal["beginner", "intermediate", "advanced", "expert"]
...
class InvestGoalPart(BaseModel):
    ...
    expected_annual_return_pct: int = Field(
        ..., ge=0, le=100, description="Expected annual rate of return (%)"
    )
...
class KnowledgeExpPart(BaseModel):
    ...
    years_of_experience: int = Field(
        ..., ge=0, le=50, description="Number of years of investment experience."
    )
...
class CapitalIncomePart(BaseModel):
    ...
    initial_capital: float = Field(
        ..., gt=0, description="Initial capital for this profile."
    )
    income_dependency: Literal["low", "medium", "high"] = Field(
...
class PersonalPreferPart(BaseModel):
    ...
    preferred_sectors: Optional[list[str]] = Field(
        default=None, description="Preferred sectors (eg: ['TECH', 'HEALTHCARE'])."
    )
    excluded_sectors: Optional[list[str]] = Field(
        default=None, description="Sectors you want to exclude."
    )
    ethical_investing: bool = Field(
        default=False, description="Prefer ethical investing (ESG) criteria."
    )
...
class ProfileBase(BaseModel):
    ...
    profile_name: str = Field(
        ..., min_length=3, max_length=100, description="Name of profile"
    )
