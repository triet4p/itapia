...
from itapia_common.schemas.entities.performance import (
    PerformanceFilterWeights,
    PerformanceHardConstraints,
)
from itapia_common.schemas.entities.personal import (
    BehaviorModifiers,
...
class QuantitivePreferencesAnalyzer:
    ...
    def _map_profile_to_weights(
        ...
        weights_dict: Dict[str, float] = {
            k: max(0, v) for k, v in weights.model_dump().items()
        }
    ...
    def _map_profile_to_constraints(
        self, profile: ProfileEntity
    ) -> PerformanceHardConstraints:
        ...
        min_requirement_factor = horizon_map.get(
            profile.invest_goal.investment_horizon, 0.05
        )
