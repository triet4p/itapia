...
class RulesOrchestrator:
    ...
    def run_single_rule(
        self, report: QuickCheckAnalysisReport, rule: Rule
        ...
        trigger_rule = TriggeredRuleInfo(
            rule_id=rule.rule_id, name=rule.name, score=score, purpose=rule.purpose
        )

        return score, trigger_rule

    def run_personal_rules(
        self,
        report: QuickCheckAnalysisReport,
    ...
    async def get_explaination_for_single_rule(
        self, rule_id: str
    ) -> ExplainationRuleEntity:
        ...
        try:
            ...
            entity = ExplainationRuleEntity(explain=explanation, **rule.model_dump())
            return entity
        except ValueError:
