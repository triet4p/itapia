...
async def get_quick_analysis_explanation(
    ...
    """
    try:
        report = await orchestrator.get_full_analysis_explaination_report(
...
async def get_technical_quick_analysis(
    ...
    """
    try:
        report = await orchestrator.get_technical_report(
            ticker, daily_analysis_type, required_type
        )
...
async def get_forecasting_quick_analysis(
    ticker: str,
    orchestrator: AIServiceQuickOrchestrator = Depends(get_ceo_orchestrator),
):
...
async def get_news_quick_analysis(
    ticker: str,
    orchestrator: AIServiceQuickOrchestrator = Depends(get_ceo_orchestrator),
):
