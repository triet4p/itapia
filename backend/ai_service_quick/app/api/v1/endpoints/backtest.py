...
from itapia_common.schemas.api.backtest import BacktestGenerationRequest, BacktestGenerationCheckResponse

router = APIRouter()

@router.post('/backtest/generate',
...
async def generate_backtest(req: BacktestGenerationRequest,
    ...
    backtest_dates = [datetime.fromtimestamp(ts, tz=timezone.utc) for ts in backtest_dates_ts]
    
    orchestrator.backtest_jobs_status[job_id] = 'IDLE'
    asyncio.create_task(orchestrator.generate_backtest_reports(job_id, req.ticker, backtest_dates))
    
    return BacktestGenerationCheckResponse(job_id=job_id, status='IDLE')
    
@router.get('/backtest/check/{job_id}', response_model=BacktestGenerationCheckResponse,
            summary="Get the current status of the backtest data generation process")
async def check_backtest(job_id: str,
                         orchestrator: AIServiceQuickOrchestrator = Depends(get_ceo_orchestrator)):
