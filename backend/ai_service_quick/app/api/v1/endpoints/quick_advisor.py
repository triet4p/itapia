...
async def get_full_advisor_report(
    quantitive_config: QuantitivePreferencesConfigRequest,
    ticker: str,
    limit: int = 10,
    ...
    try:
        report = await orchestrator.get_full_advisor_report(
            ticker=ticker,
            quantitive_config=QuantitivePreferencesConfig.model_validate(
                quantitive_config.model_dump()
            ),
            limit=limit,
        )
...
async def get_full_advisor_explaination_report(
    ...
    """
    try:
        report = await orchestrator.get_full_advisor_explaination_report(
            ticker=ticker,
            quantitive_config=QuantitivePreferencesConfig.model_validate(
                quantitive_config.model_dump()
            ),
