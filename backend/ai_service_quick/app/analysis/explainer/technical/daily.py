...
class _KeyIndicatorsExplainer:
    ...
    def explain(self, indicators: KeyIndicators) -> str:
        ...
        if (
            indicators.rsi_14 is None
            or indicators.bbl_20 is None
            or indicators.bbu_20 is None
        ):
            return "Key indicator data is incomplete for a full explanation."

        return self._TEMPLATE.format(
            rsi_14=indicators.rsi_14, bbl_20=indicators.bbl_20, bbu_20=indicators.bbu_20
        )
...
class _MidTermTrendExplainer:
    ...
    def explain(self, report: MidTermTrendReport) -> str:
        return self._TEMPLATE.format(
            ma_direction=report.ma_direction, adx_direction=report.adx_direction
        )
...
class _OverallStrengthTrendExplainer:
    ...
    def explain(self, report: OverallStrengthTrendReport) -> str:
        return self._TEMPLATE.format(strength=report.strength, value=report.value)
...
class _SRExplainer:
    ...
    def explain(self, report: SRReport) -> str:
        ...
        return self._TEMPLATE.format(
            supports_str=supports_str, resistances_str=resistances_str
        )
...
class _PatternExplainer:
    ...
    def _build_patterns_summary(self, patterns: List[PatternObj]) -> str:
        ...
        for pattern in patterns[:2]:
            ...
            summaries.append(
                f"{sentiment_text} '{pattern.name}' ({pattern.pattern_type}) pattern"
            )
...
class DailyAnalysisExplainer:
    ...
    def explain(self, report: DailyAnalysisReport) -> str:
        key_indicators_exp = self.key_indicators_explainer.explain(
