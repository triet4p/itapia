...
class _FeatureEngine:
    ...
    def get_features(
        self,
        copy: bool = True,
        handle_na_method: Optional[str] = "forward_fill",
    ...
    def _add_generic_indicator(
        ...
        if configs is None:
            ...
            if configs is None:
                logger.warn(
                    f"Warning: No default config for '{indicator_name}'. Skipping."
                )
                return self
        try:
        ...
        for config in configs:
            ...
            if invalid_keys:
                ...
                logger.warn(
                    f"Warning: Invalid parameter(s) {invalid_keys} for '{indicator_name}' with config {config}. "
...
class DailyFeatureEngine(_FeatureEngine):
    ...
    # --- GROUP METHODS ---
    def _add_indicator_group(
        self,
        group_name: str,
        indicators: List[str],
        all_configs: Optional[Dict[str, List[Dict[str, any]]]] = None,
    ):
    ...
    def add_trend_indicators(
        ...
        Returns:
            Self: Returns self for method chaining
        """
        return self._add_indicator_group(
            "Trend", ["sma", "ema", "macd", "adx", "psar"], all_configs
        )
    ...
    def add_momentum_indicators(
        ...
        """
        return self._add_indicator_group(
            "Momentum", ["rsi", "stoch", "cci", "willr"], all_configs
        )
    ...
    def add_volatility_indicators(
        ...
        """
        return self._add_indicator_group(
            "Volatility", ["bbands", "atr", "donchian"], all_configs
        )
    ...
    def add_diff_from_sma(self, configs: Optional[List[Dict[str, int]]] = None):
        ...
        if configs is None:
            configs = [{"sma_length": 50}, {"sma_length": 200}]
        ...
        for config in configs:
            ...
            if sma_length:
                ...
                if sma_col_name not in self.df.columns:
                    logger.warn(
                        f"Warning: Column '{sma_col_name}' not found. "
                ...
                self.df[new_col_name] = (
                    (self.df["close"] - self.df[sma_col_name])
                    / (self.df[sma_col_name] + 1e-9)
                    * 100
                )
    ...
    def add_return_d(self, configs: Optional[List[Dict[str, int]]] = None):
        ...
        if configs is None:
            configs = [{"d": 1}, {"d": 5}, {"d": 20}]
        for config in configs:
            d_period = config.get("d")
            if d_period:
                self.df[f"return_{d_period}d"] = (
                    self.df["close"].pct_change(periods=d_period) * 100
                )
        return self
    ...
    def add_lag_features(self, configs: Optional[List[Dict[str, any]]] = None):
        ...
        for config in configs:
            ...
            if not col_to_lag or not lag_periods:
                logger.warn(
                    f"Warning: Invalid lag config {config}. Missing 'column' or 'periods'. Skipping."
                )
                continue
            ...
            if col_to_lag not in self.df.columns:
                logger.warn(
                    f"Error: Column '{col_to_lag}' not found for lagging. "
    ...
    def add_interaction_features(self):
        ...
        if "CDL_HAMMER" in self.df.columns and "RSI_14" in self.df.columns:
            ...
            self.df["hammer_in_oversold"] = (self.df["CDL_HAMMER"] / 100) * self.df[
                "is_oversold"
            ]
    ...
    def add_all_features(
        ...
        (
            ...
            .add_diff_from_sma(
                all_configs.get("diff_from_sma") if all_configs else None
            )
...
class IntradayFeatureEngine(_FeatureEngine):
    ...
    def add_intraday_momentum(
        self,
        rsi_configs: Optional[List[Dict]] = None,
        macd_configs: Optional[List[Dict]] = None,
    ):
    ...
    def add_intraday_volatility(
        self,
        bbands_configs: Optional[List[Dict]] = None,
        atr_configs: Optional[List[Dict]] = None,
    ):
    ...
    def add_opening_range(self, minutes: int = 30):
        ...
        """
        logger.debug(
            f"Intraday Feature Engine: Adding {minutes}-minute Opening Range..."
        )
        ...
        def get_or(x, minutes):
            ...
            end_time = (
                pd.to_datetime(f"1970-01-01 {start_time}")
        ...
        self.df[f"OR_{minutes}m_High"] = self.df.index.to_series().dt.date.map(
            or_high_series
        )
        self.df[f"OR_{minutes}m_Low"] = self.df.index.to_series().dt.date.map(
            or_low_series
        )
    ...
    def add_all_intraday_features(self):
        ...
        """
        logger.info(
            "Intraday Feature Engine: Adding all standard intraday features ..."
        )
        (
            self.add_intraday_ma()
