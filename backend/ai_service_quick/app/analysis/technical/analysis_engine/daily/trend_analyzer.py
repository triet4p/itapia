...
class DailyTrendAnalyzer:
    ...
    def _get_adx_strength(self, adx_col: str) -> OverallStrengthTrendReport:
        ...
        if adx_col not in self.latest_row:
            return OverallStrengthTrendReport(strength="undefined", value=0)

        adx_val = self.latest_row[adx_col]
        if adx_val > 25:
            return OverallStrengthTrendReport(strength="strong", value=adx_val)
        elif adx_val >= 20:
            return OverallStrengthTrendReport(strength="moderate", value=adx_val)
        else:
            return OverallStrengthTrendReport(strength="weak", value=adx_val)
    ...
    def _get_adx_direction(self, dmp_col: str, dmn_col: str) -> str:
        ...
        return (
            "uptrend"
            if self.latest_row[dmp_col] > self.latest_row[dmn_col]
            else "downtrend"
        )
    ...
    def _get_long_term_view(self) -> LongTermTrendReport:
        ...
        return LongTermTrendReport(
            ma_direction=trend_direction, ma_status=status, evidence=evidence
        )
