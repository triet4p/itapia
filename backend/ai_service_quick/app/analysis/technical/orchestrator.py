...
class TechnicalOrchestrator:
    ...
    def get_daily_features(self, ohlcv_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            return engine.add_all_features().get_features(
    ...
    def get_intraday_features(self, ohlcv_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            return engine.add_all_intraday_features().get_features(
                handle_na_method="forward_fill", reset_index=False
            )
        except (ValueError, TypeError) as e:
    ...
    def get_full_analysis(
        ...
        if required_type == "daily" or required_type == "all":
            daily_report = self.get_daily_analysis(
                enriched_daily_df, daily_analysis_type
            )
    ...
    def get_full_past_analysis(
        self, enriched_daily_df: pd.DataFrame, current_date_enriched: pd.Series
    ) -> TechnicalReport:
        ...
        """
        daily_report = self.get_daily_analysis(
            enriched_df=enriched_daily_df, analysis_type="medium"
        )
        intraday_mock_report = IntradayAnalysisEngine.get_mock_report(
            current_date_enriched
        )
