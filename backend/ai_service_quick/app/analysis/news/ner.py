...
class TransformerNERModel:
    ...
    def __init__(self, model_name: str, ner_score_threshold: float):
        ...
        """
        self.pipe = pipeline(
            "ner", model=model_name, device="cpu", aggregation_strategy="average"
        )
        ...
        for res in entities_results:
            ...
            for entity in res:
                if float(entity["score"]) < self.ner_score_threshold:
                    continue
                elements.append(
                    NERElement(entity_group=entity["entity_group"], word=entity["word"])
                )
            reports.append(NERReport(entities=elements))
        return reports
...
class SpacyNERModel:
    ...
    def __init__(self, model_name: str):
        ...
        try:
            ...
            print(
                f"Spacy model '{self.model_name}' not found. Attempting to download automatically..."
            )
            self._download_model()
            # After downloading, try to load again
            self.nlp: Language = spacy.load(self.model_name)
            print(
                f"Successfully downloaded and loaded spaCy model '{self.model_name}'."
            )
    ...
    def recognize(self, texts: List[str]) -> List[NERReport]:
        ...
        for doc in docs:
            found_entities = [
                NERElement(word=ent.text, entity_group=ent.label_) for ent in doc.ents
            ]
