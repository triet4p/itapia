...
def _create_impact_assessment_model_sync() -> WordBasedImpactAssessmentModel:
    ...
    """
    try:
        high_impact_dictionary = load_dictionary(
            cfg.NEWS_IMPACT_DICTIONARY_PATH.format(level="high")
        )
    except FileNotFoundError:
        logger.warn("Not found dictionary file, return empty dictionary")
        high_impact_dictionary = set()
    try:
        moderate_impact_dictionary = load_dictionary(
            cfg.NEWS_IMPACT_DICTIONARY_PATH.format(level="moderate")
        )
    except FileNotFoundError:
        logger.warn("Not found dictionary file, return empty dictionary")
        moderate_impact_dictionary = set()
    try:
        low_impact_dictionary = load_dictionary(
            cfg.NEWS_IMPACT_DICTIONARY_PATH.format(level="low")
        )
    except FileNotFoundError:
        logger.warn("Not found dictionary file, return empty dictionary")
        low_impact_dictionary = set()
    return WordBasedImpactAssessmentModel(
        high_impact_dictionary, moderate_impact_dictionary, low_impact_dictionary
    )
...
def _create_keyword_highlighting_model_sync() -> WordBasedKeywordHighlightingModel:
    ...
    try:
        positive_dictionary = load_dictionary(
            cfg.NEWS_KEYWORD_HIGHLIGHT_PATH.format(direction="positive")
        )
    except FileNotFoundError:
    ...
    try:
        negative_dictionary = load_dictionary(
            cfg.NEWS_KEYWORD_HIGHLIGHT_PATH.format(direction="negative")
        )
    except FileNotFoundError:
    ...
    return WordBasedKeywordHighlightingModel(
        positive_dictionary=positive_dictionary, negative_dictionary=negative_dictionary
    )
...
class NewsOrchestrator:
    ...
    async def generate_report(
        ...
        sentiment_model: SentimentAnalysisModel = await self._get_or_load_element(
            "sentiment"
        )
        ner_model: SpacyNERModel = await self._get_or_load_element("ner")
        impact_model: WordBasedImpactAssessmentModel = await self._get_or_load_element(
            "impact-assessment"
        )
        keyword_model: WordBasedKeywordHighlightingModel = (
            await self._get_or_load_element("keyword-highlight")
        )
    ...
    async def _get_or_load_element(
        ...
        async def model_factory():
            ...
            loop = asyncio.get_running_loop()
            return await loop.run_in_executor(None, func_with_params)
    ...
    async def preload_caches(self):
        ...
        for result, element_type in zip(results, self.TO_CACHE_ELEMENTS.keys()):
            if isinstance(result, Exception):
                logger.err(
                    f"A critical error during pre-warming of element '{element_type}': {result}"
                )
                failed_elements.append(element_type)

        # FIX: If any errors occurred, raise an exception
        if failed_elements:
            raise PreloadCacheError(
                module_name="News Analysis", failed_elements=failed_elements
            )
