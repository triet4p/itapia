...
def get_ensemble_feature_ranks(
    X: pd.DataFrame, y: pd.Series, task_type: Literal["clf", "reg"]
) -> pd.DataFrame:
...
def get_ranked_features(
    ...
    for col in score_cols:
        ...
        ranks_df[f"{col}_norm"] = (
            (ranks_df[col] - min_val) / (max_val - min_val)
            if max_val > min_val
            else 0.5
        )
    ...
    ranks_df["final_score_raw"] = np.average(
        ranks_df[norm_score_cols],
        axis=1,
        weights=[weights[sc.split("_")[0]] for sc in score_cols],
    )
...
def select_k_plus_l_features(
    ranks_df: pd.DataFrame, k=45, l=5, cdl_prefix="CDL_"
) -> List[str]:
