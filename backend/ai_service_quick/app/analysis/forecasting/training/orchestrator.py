...
class TrainingOrchestrator:
    ...
    def _create_id_for_orchestrate(
        self, model: ForecastingModel, task: ForecastingTask
    ) -> str:
    ...
    def run_feature_selection(
        self, weights: Dict, bonus_features: List[str], bonus_multiplier: float
    ):
        ...
        for id, model in self.models_to_train.items():
            ...
            ranked_df = get_ranked_features(
                ranks_df, weights, bonus_features, bonus_multiplier
            )

            selected = select_k_plus_l_features(
                ranked_df, task.non_cdl_features_cnt, task.cdl_features_cnt
            )
    ...
    def split_data(
        self, train_test_split_date: datetime, test_last_date: datetime = datetime.now()
    ):
        ...
        if self.df_with_targets is None:
            raise RuntimeError(
                "Targets must be prepared before splitting data. Run `prepare_all_targets()` first."
            )
        self._train_df, self._test_df = train_test_split(
            self.df_with_targets, train_test_split_date, test_last_date
        )
        print(
            f"Train set shape: {self._train_df.shape}, Test set shape: {self._test_df.shape}"
        )

    def run_walk_forward_validation(
        self, validation_months: int, max_train_months: int | None = None
    ):
        ...
        if self._train_df is None:
            raise RuntimeError(
                "Data must be split before running validation. Run `split_data()` first."
            )
        ...
        for id, model in self.models_to_train.items():
            ...
            evaluation_results = []
            generator = get_walk_forward_splits(
                self._train_df, validation_months, max_train_months=max_train_months
            )
            ...
            for i, (train_fold_df, valid_fold_df) in enumerate(generator):
                ...
                y_train = (
                    ...
                    else y_train_df.values
                )
                y_valid = (
                    y_valid_df.values.ravel()
                    if task.task_type == "clf"
                    else y_valid_df.values
                )
                ...
                if task.task_type == "clf":
                    ...
                    score = math.sqrt(
                        mean_squared_error(
                            y_valid, preds, multioutput="uniform_average"
                        )
                    )
            ...
            print(
                f"  - Finished validation. Collected {len(model.snapshot_models)} snapshots."
            )
    ...
    def run_final_training_and_registration(self, kaggle_username: str):
        ...
        if self._train_df is None:
            raise RuntimeError(
                "Data must be split before final training. Run `split_data()` first."
            )
        ...
        for task_id, model in self.models_to_train.items():
            ...
            y_final_train = (
                y_final_train_df.values.ravel()
                if task.task_type == "clf"
                else y_final_train_df.values
            )

            X_final_test = self._test_df[task.selected_features]
            y_final_test_df = self._test_df[task.targets]
            y_final_test = (
                y_final_test_df.values.ravel()
                if task.task_type == "clf"
                else y_final_test_df.values
            )
            ...
            if task.task_type == "clf":
                ...
                score = math.sqrt(
                    mean_squared_error(
                        y_final_test, y_pred_test, multioutput="uniform_average"
                    )
                )
                metric_name = "avg_rmse"
