...
def train_test_split(
    df: pd.DataFrame, train_test_split_date: datetime, test_last_date: datetime
...
def get_walk_forward_splits(
    ...
    print(
        f"Generating splits from {start_year} to {end_year} with {validation_months}-month validation..."
    )
    if max_train_months:
    ...
    for year in range(start_year, end_year + 1):
        ...
        if valid_df.empty:
            print(
                f"  - Skipping validation for year {year}: no data in the last {validation_months} months."
            )
            continue
        ...
        if max_train_months is not None:
            # Sliding Window mode
            train_start_date = validation_start_date - pd.DateOffset(
                months=max_train_months
            )
            train_df = df[
                (df.index >= train_start_date) & (df.index < validation_start_date)
            ]
        else:
        ...
        if not train_df.empty:
            ...
            print(
                f"    - Train: {train_df.index.min().date()} -> {train_df.index.max().date()} (shape: {train_df.shape})"
            )
            print(
                f"    - Valid: {valid_df.index.min().date()} -> {valid_df.index.max().date()} (shape: {valid_df.shape})"
            )
