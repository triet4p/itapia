...
class LGBMClassifierObjective(OptunaObjective):
    ...
    def __init__(
        ...
        """
        super().__init__(
            model, full_df, direction, generator, time_weighted, weight_bias, max_cv
        )
    ...
    def __call__(self, trial) -> float:
        ...
        params = {
            ...
            ),  # Or 'binary' depending on the problem
            ...
            "min_split_gain": trial.suggest_float(
                "min_split_gain", 1e-8, 0.1, log=True
            ),
            # loguniform(1e-4, 0.1) -> suggest_float(low, high, log=True)
            "min_child_weight": trial.suggest_float(
                "min_child_weight", 1e-4, 0.1, log=True
            ),
            ...
            "colsample_bytree": trial.suggest_float(
                "colsample_bytree", 0.25, 1.0, log=True
            ),
...
def get_best_params_for_kernel_model(obj: OptunaObjective, n_trials: int = 500) -> dict:
