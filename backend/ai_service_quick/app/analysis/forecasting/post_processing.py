...
from .task.ndays_distribution import DISTRIBUTION_FEATURES, NDaysDistributionTask
...
class NDaysDistributionPostProcessor(PostProcessor):
    ...
    def apply(self, raw_predict):
        ...
        if min_greater_max.any():
            mid = (
                raw_df.loc[min_greater_max, "min"] + raw_df.loc[min_greater_max, "max"]
            ) / 2
        ...
        if q25_greater_q75.any():
            mid_q = (
                raw_df.loc[q25_greater_q75, "q25"] + raw_df.loc[q25_greater_q75, "q75"]
            ) / 2
