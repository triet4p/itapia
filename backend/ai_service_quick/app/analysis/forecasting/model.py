...
from .task import AvailableTaskTemplate, ForecastingTask, ForecastingTaskFactory
...
class ForecastingModel(ABC):
    ...
    def get_model_slug(self, task_id: str = None) -> str:
        ...
        if task_id is not None:
            return cfg.MODEL_SLUG_TEMPLATE.format(
                id=f"{self.name}-{task_id.lower().replace('_','-')}"
            )
        return cfg.MODEL_SLUG_TEMPLATE.format(
            id=f"{self.name}-{self.task.task_id.lower().replace('_','-')}"
        )
    ...
    def register_model_to_kaggle(
        ...
        try:
            ...
            if not version_notes:
                version_notes = (
                    f"Training run on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )

            kagglehub.model_upload(
                handle=handle, local_model_dir=artifact_dir, version_notes=version_notes
            )
            print(
                f"[{self.name}] Successfully uploaded a new version to handle: {handle}"
            )
    ...
    def load_model_from_kaggle(
        ...
        try:
            ...
            if os.path.exists(main_model_path):
                ...
                print(
                    f"  - Successfully loaded main kernel model: {type(self.kernel_model)}"
                )
            else:
                raise FileNotFoundError(
                    f"Main model file '{cfg.MODEL_MAIN_MODEL_FILE}' not found in downloaded artifacts."
                )
            ...
            if os.path.exists(metadata_path):
                ...
                if task_metadata:
                    self.task = ForecastingTaskFactory.create_task(
                        task_template, task_id, task_metadata
                    )
                else:
                    print(
                        "Warning: 'task' key not found in metadata.json. Task state not restored."
                    )

                self.snapshot_registry = full_metadata.get("snapshots", {}).get(
                    "details", {}
                )
            else:
            ...
            print(
                f"[{self.name}] Model loading complete with task {self.task.task_id}."
            )

        except Exception as e:
            print(
                f"[{self.name}] An error occurred while loading model from Kaggle Hub: {e}"
            )
            raise
        finally:
            print(f"[{self.name}] Cleaning up temporary artifact directory...")
    ...
    def fit_kernel_model(
        self, kernel_model, X: pd.DataFrame, y: pd.Series | pd.DataFrame
    ) -> None:
        """
        Fit a kernel model.
        """
    ...
    def predict(self, X: pd.DataFrame, snapshot_id: str | None = None) -> np.ndarray:
        ...
        if snapshot_id is not None:
            ...
            if self.kernel_model is None:
                raise TypeError(
                    "Main kernel model has not been trained yet. Call fit() without a snapshot_id."
                )
    ...
    def get_snapshot_by_test_time(
        self, test_time: datetime, match_type: Literal["first", "last"] = "last"
    ) -> str:
        """
    ...
    def load_all_snapshot_from_disk(self):
        if self.model_cache_path is None:
            raise FileNotFoundError(
                "  - No snapshots directory found, skipping snapshot loading."
            )
        self.snapshot_models = {}
        ...
        if os.path.exists(snapshot_dir):
            ...
            print(
                f"  - Successfully loaded {len(self.snapshot_models)} snapshot models."
            )
        else:
            raise FileNotFoundError(
                "  - No snapshots directory found, skipping snapshot loading."
...
class ScikitLearnForecastingModel(ForecastingModel):
    def __init__(
        ...
        super().__init__(
            name,
            kernel_model_template=kernel_model_template,
