...
def get_triple_barrier_labels(
    prices: pd.Series, h: int, tp_pct: float, sl_pct: float
) -> pd.Series:
...
def find_triple_barrier_optimal_params(
    ...
    print(
        f"--- Generating Grid result with {len(horizons)*len(tp_pcts)*len(sl_pcts)} candidates ---"
    )
    cnt = 0
    for h in horizons:
        for tp in tp_pcts:
            for sl in sl_pcts:
                ...
                train_labels_list = [
                    get_triple_barrier_labels(group[base_price_col], h, tp, sl)
                    for _, group in df_train.groupby("ticker")
                ]
                train_labels = pd.concat(train_labels_list).dropna()

                test_labels_list = [
                    get_triple_barrier_labels(group[base_price_col], h, tp, sl)
                    for _, group in df_test.groupby("ticker")
                ]
                ...
                results.append(
                    {
                        ...
                        "tp_pct": tp,
                        "sl_pct": sl,
                        "win_train": train_dist.get(1, 0),
                        "loss_train": train_dist.get(-1, 0),
                        "timeout_train": train_dist.get(0, 0),
                        "win_test": test_dist.get(1, 0),
                        "loss_test": test_dist.get(-1, 0),
                        "timeout_test": test_dist.get(0, 0),
                    }
                )

                if cnt % 10 == 0:
                    print(
                        f"--- Generated {cnt}/{len(horizons)*len(tp_pcts)*len(sl_pcts)} candidates"
                    )
    ...
    results_df["balance_score"] = results_df[["win_train", "loss_train"]].min(
        axis=1
    ) / results_df[["win_train", "loss_train"]].max(axis=1)
    ...
    win_diff = abs(results_df["win_test"] - results_df["win_train"]) / (
        results_df["win_train"] + 1e-9
    )
    loss_diff = abs(results_df["loss_test"] - results_df["loss_train"]) / (
        results_df["loss_train"] + 1e-9
    )
    ...
    results_df["rr_score"] = (rr.clip(lower=min_rr, upper=max_rr) - min_rr) / (
        max_rr - min_rr
    )

    # 5. Horizon Score - Prefer larger horizons
    # Use a linear or log function to normalize horizon to [0, 1] scale
    results_df["horizon_score"] = (
        results_df["h"]
        .apply(lambda x: 1 if x in [10, 15] else 1 - (max(10 - x, x - 15)) / 10)
        .clip(lower=0)
    )
...
class TripleBarrierTask(ForecastingTask):
    ...
    def __init__(
        ...
        """
        super().__init__(
            task_id, "clf", "category", require_cdl_features, require_non_cdl_features
        )
    ...
    def create_targets(self, df: pd.DataFrame, base_price_col: str) -> pd.DataFrame:
        ...
        if "ticker" not in df.columns:
            raise ValueError(
                "DataFrame must have a 'ticker' column for group-aware target creation."
            )

        all_labels = []
        for _, group in df.groupby("ticker"):
            labels = get_triple_barrier_labels(
                group[base_price_col], self.horizon, self.tp_pct, self.sl_pct
            )
