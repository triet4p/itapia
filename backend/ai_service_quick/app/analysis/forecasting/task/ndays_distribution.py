...
def create_distribution_targets(prices: pd.Series, horizon: int) -> pd.DataFrame:
    ...
    targets.iloc[:num_predictions, 4] = np.quantile(
        windows_for_prediction, 0.25, axis=1
    )
    targets.iloc[:num_predictions, 5] = np.quantile(
        windows_for_prediction, 0.75, axis=1
    )
...
class NDaysDistributionTask(ForecastingTask):
    ...
    def __init__(
        ...
        """
        super().__init__(
            task_id, "reg", "percent", require_cdl_features, require_non_cdl_features
        )
    ...
    def get_metadata_for_plain(self) -> NDaysDistributionTaskMetadata:
        ...
        return NDaysDistributionTaskMetadata(
            targets=self.targets, units=self.target_units, horizon=self.horizon
        )
        ...
        for _, group in df.groupby("ticker"):
            ...
            for stat in ["mean", "min", "max", "q25", "q75"]:
                ...
                norm_targets[f"target_{stat}_pct_{self.horizon}d"] = (
                    (abs_targets[col_name] - current_close) / current_close
                ) * 100
            ...
            norm_targets[f"target_std_pct_{self.horizon}d"] = (
                abs_targets[col_std] / current_close
            ) * 100
