...
class AggregationOrchestrator:
    ...
    def map_final_scores(
        ...
        """
        dec_desc = self.mapper.map(
            final_scores[SemanticType.DECISION_SIGNAL], SemanticType.DECISION_SIGNAL
        )
        risk_desc = self.mapper.map(
            final_scores[SemanticType.RISK_LEVEL], SemanticType.RISK_LEVEL
        )
        opp_desc = self.mapper.map(
            final_scores[SemanticType.OPPORTUNITY_RATING],
