...
def sample_intraday_data() -> pd.DataFrame:
    ...
    """
    # Ngày 1
    dates1 = pd.to_datetime(pd.date_range(start="2024-05-20 09:30", end="2024-05-20 16:00", freq="15min"))
    # Ngày 2
    dates2 = pd.to_datetime(pd.date_range(start="2024-05-21 09:30", end="2024-05-21 16:00", freq="15min"))
...
def test_vwap_resets_daily(sample_intraday_data):
    ...
    expected_vwap_day1 = (first_candle_day1['high'] + first_candle_day1['low'] + first_candle_day1['close']) / 3
    assert np.isclose(first_candle_day1['VWAP_D'], expected_vwap_day1)
    
    # Ngày 2
    first_candle_day2 = df.loc["2024-05-21 09:30:00"]
    expected_vwap_day2 = (first_candle_day2['high'] + first_candle_day2['low'] + first_candle_day2['close']) / 3
