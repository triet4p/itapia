...
# Import lớp cần test
from app.analysis.technical.analysis_engine.daily.pattern_recognizer import (
    DailyPatternRecognizer,
)


# --- Hàm Helper để tạo dữ liệu có mẫu hình ---
def create_pattern_df(
    price_sequence: List[int], cdl_data: Dict[str, int] = None
) -> pd.DataFrame:
    """
...
def test_is_double_top_confirmed():
    ...
    recognizer = DailyPatternRecognizer(
        df, history_window=7, prominence_pct=0.01, distance=1
    )
...
def test_find_extrema_correctness():
    ...
    recognizer = DailyPatternRecognizer(
        df, history_window=len(price_seq), prominence_pct=0.01, distance=1
    )
