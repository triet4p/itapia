...
# Import lớp cần test
from app.analysis.technical.analysis_engine.daily.trend_analyzer import (
    DailyTrendAnalyzer,
)
...
def test_long_term_uptrend():
    ...
    latest_row = pd.Series(
        {"close": 150, "SMA_50": 140, "SMA_200": 120}  # SMA_50 > SMA_200
    )
...
def test_adx_strength_analysis():
    ...
    analyzer = DailyTrendAnalyzer(
        pd.DataFrame([{"ADX_14": -1}], index=[pd.to_datetime("2025-01-01")])
    )  # Khởi tạo tạm
