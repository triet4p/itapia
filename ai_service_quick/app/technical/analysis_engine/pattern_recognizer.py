...
_chart_pattern_registry: Dict[str, Callable[..., bool]] = {}
...
class PatternRecognizer:
    ...
    def _find_candlestick_patterns(self) -> List[str]:
        patterns = []
        ...
        for col, value in latest_row_lower.items():
            ...
            if col.startswith('cdl_') and value != 0:
                ...
                prefix = "Bullish" if value > 0 else "Bearish"
                patterns.append(f"{prefix} {pattern_name}")
        return patterns
    
    def _find_chart_patterns(self) -> List[str]:
        found_patterns = []
        for pattern_name, checker_function in _chart_pattern_registry.items():
            if checker_function(self):
    ...
    def _is_double_top(self, tolerance: float = 0.015) -> bool:
        if len(self.peaks) < 2 or len(self.troughs) < 1: return False
        ...
        if troughs_between.empty: return False
        ...
        return p1.price > neckline.price and p2.price > neckline.price and height_similarity and is_confirmed
    ...
    def _is_double_bottom(self, tolerance: float = 0.015) -> bool:
        ...
        is_confirmed = self.latest_row['close'] > neckline.price
        return t1.price < neckline.price and t2.price < neckline.price and depth_similarity and is_confirmed
    ...
    def _is_head_and_shoulders(self, tolerance: float = 0.015) -> bool:
        ...
        return head_is_highest and shoulders_are_similar and is_confirmed