...
class AIServiceQuickOrchestrator:
    ...
    def get_full_analysis_report(self, ticker: str):
        ...
        """
        print(f"--- CEO: Initiating full analysis for ticker '{ticker}' ---")
        
        # --- BƯỚC 1: LẤY DỮ LIỆU THÔ ---
        # Ra lệnh cho "Trưởng phòng Chuẩn bị Dữ liệu"
        print("CEO -> DataPreparer: Fetching daily and intraday data...")
        ...
        print(f"--- CEO: Full analysis for '{ticker}' complete. ---")
        return final_report
    ...
    def prepare_training_data_for_sector(self, sector_code: str) -> pd.DataFrame:
        ...
        """
        print(f"--- CEO: Preparing training data for sector '{sector_code}' ---")
        
        # BƯỚC 1: LẤY DỮ LIỆU GỘP CỦA CẢ NGÀNH
        print("CEO -> DataPreparer: Fetching and transforming sector data...")
        ...
        print("CEO -> TechAnalyzer: Generating features for sector data...")
        all_enriched_dfs = []
        ...
        print(f"--- CEO: Training data for '{sector_code}' is ready. ---")
