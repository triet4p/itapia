...
def get_daily_prices_by_sector(sector: str, skip: int = 0, limit: int = 2000,
                               data_service: DataService = Depends(get_data_service)):
    """
    Lấy dữ liệu giá lịch sử hàng ngày cho tất cả các cổ phiếu
    thuộc một nhóm ngành cụ thể (dựa trên sector_code).
    
    Trả về một danh sách, mỗi phần tử là dữ liệu của một cổ phiếu.
    """
...
def get_all_sectors(data_service: DataService = Depends(get_data_service)):
    """
    Lấy danh sách đầy đủ tất cả các nhóm ngành được hỗ trợ trong hệ thống.
    Hữu ích cho việc xây dựng các bộ lọc hoặc dropdown trên giao diện người dùng.
    """
